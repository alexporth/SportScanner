...
try:
    ...
    sportsdb = ''
try:
    sportsdb = ''.join(map(str, [chr(ord(a) ^ ord(b))
                                 for a, b in zip(base64.b64decode(thesportsdb.sportsdbkey1),
                                                 base64.b64decode(thesportsdb.sportsdbkey2))]))
...
def get_result_from_network(url, fetch_content=True):
...
def get_league_details(league_id, cache=None):
    if cache is not None:
        if league_id in cache:  # Check if a cached value exists.
            # Cached value exists. Use it.
            return cache[league_id]
    # Nothing in cache or no cache provided.
    url = "{0}lookupleague.php?id={1}".format(SPORTSDB_API, league_id)
    try:
        details = (JSON.ObjectFromString(get_result_from_network(url, True)))['leagues'][0]
    except:
...
class SportScannerAgent(Agent.TV_Shows):
    ...
    def search(self, results, media, lang, manual):
        ...
        try:
            potential_leagues = (JSON.ObjectFromString(get_result_from_network(url, True)))['leagues']
        except:
        ...
        # Iterate through all of the sports primary names.
        # If this fails then we can make a call for each sport and match against alternate names.
        if potential_leagues is not None:
            for league in potential_leagues:  # So far we've only made 1 API call to TSDB.
                if show_title == league['strLeague']:
                    ...
                    league_details = get_league_details(league['idLeague'],
        ...
        if not match:
            if potential_leagues is not None:
                for league_index in range(0, len(potential_leagues)):
                    # Log("SS: Comparing {0] to {1}".format(x['strLeague'], show_title))
                    # Get the full details of the league
                    league_details = get_league_details(potential_leagues[league_index]['idLeague'], self.cached_leagues)
        ...
        if not match and manual:
            ...
            for league_index in range(0, len(potential_leagues)):
                # Get league details
                league_details = get_league_details(potential_leagues[league_index]['idLeague'], self.cached_leagues)
                self.cached_leagues[potential_leagues[league_index]['idLeague']] = league_details
    ...
    def update(self, metadata, media, lang, force):
        ...
        try:
            ...
            league_metadata = JSON.ObjectFromString(get_result_from_network(url, True))['leagues'][0]
        except:
            pass
            ...
            for s in media.seasons:
                for e in media.seasons[s].episodes:
                    ...
                    def UpdateEpisode(
                        ...
                        additional_metadata_file = '{}.SportScanner'.format(
                            os.path.splitext(episode_media.items[0].parts[0].file)[0])
                        ...
                        if matched_episode is None:
                            ...
                            if match:
                                # Replace all whitespaces with underscores for league name
                                league_name = re.sub(r'\s+', '_', league_metadata['strLeague'])
                                url = "{0}eventsday.php?d={1}-{2}-{3}&l={4}".format(
                                    SPORTSDB_API,
                                    match.group('year'),
                                    match.group('month'),
                                    match.group('day'),
                                ...
                                day_events = JSON.ObjectFromString(get_result_from_network(url, True))
                                ...
                                for current_event in range(len(day_events['events'])):
                                    ...
                                    if (("dateEvent" in day_events['events'][current_event])
                                            and hasattr(episode_media, 'originally_available_at')):
                                        if (day_events['events'][current_event]['dateEvent']
                                                == episode_media.originally_available_at):
                                            total_matches += 1
                                            closeness = similar(
                                                episode_media.title, day_events['events'][current_event]['strEvent'])
                                            # Take a second closeness with bastard name
                                            bastard_title = "{0} {1}".format(metadata.title, episode_media.title)
                                            bastard_closeness = similar(
                                                bastard_title, day_events['events'][current_event]['strEvent'])
                            ...
                            # Only accept if the match is better than 80% or
                            # assume we have found the correct event if there is only one on that date
                        ...
                        if len(additional_metadata_subtitle) > 0:
                            episode.title = '{0} {1}'.format(matched_episode['strEvent'], additional_metadata_subtitle)
                        else:
                            episode.title = matched_episode['strEvent']
                        # Generate a useful description based on the available fields
                        extra_details = ""
                        if (matched_episode.get('strAwayTeam') is not None
                                and matched_episode.get('strHomeTeam') is not None):
                            extra_details = "{0} vs. {1}\n".format(
                                matched_episode['strHomeTeam'], matched_episode['strAwayTeam'])
                        if matched_episode.get('dateEvent') is not None and matched_episode.get('strTime') is not None:
                            extra_details = "{0}Played on {1} at {2}\n".format(
                                extra_details, matched_episode['dateEvent'], matched_episode['strTime'])
                        if matched_episode.get('strCircuit') is not None:
                            extra_details = "{0}Race venue: {1}".format(
                                extra_details, matched_episode['strCircuit'])
                            if matched_episode.get('strCountry') is not None:
                                if matched_episode.get('strCity') is not None:
                                    extra_details = "{0} in {1}, {2}".format(
                                        extra_details, matched_episode['strCity'], matched_episode['strCountry'])
                                else:
                                    extra_details = "{0} in {1}".format(
                                        extra_details, matched_episode['strCountry'])
                        summary = "{0}\n\n{1}\n\nMatched by SportScanner.".format(
                            extra_details, matched_episode['strDescriptionEN'])
                        ...
                        if matched_episode.get('strThumb') is not None:
                            ...
                            if thumb not in episode.thumbs or force:
                                try:
                                    episode.thumbs[thumb] = Proxy.Media(get_result_from_network(thumb, True))
        ...
        def DownloadImages():
            ...
            if league_metadata.get('strPoster') is not None:
                ...
                for banner_index in range(1, 10):
                    key_name = "strPoster{0}".format(banner_index)
                ...
                for poster_index in range(len(posters_to_dl)):
                    poster_url = posters_to_dl[poster_index]
                    if poster_url not in metadata.posters:
                        Log("SS: Downloading poster: {0}".format(poster_url))

                        @task
                        def DownloadImage(metadata=metadata, poster_url=poster_url, i=poster_index):
                            if poster_url not in metadata.posters:
                                Log("SS: Downloading poster {0}".format(poster_url))
                                try:
                                    metadata.posters[poster_url] = Proxy.Preview(
                                        get_result_from_network(poster_url, False),
                                        sort_order=(i + 1))
                                except:
            ...
            if league_metadata.get('strBanner') is not None:
                ...
                for banner_string_index in range(1, 10):
                    key_name = "strBanner{0}".format(banner_string_index)
                    if key_name in league_metadata:
                        if league_metadata.get(key_name) is not None:
                            banners_to_dl.append(league_metadata[key_name])
                            # banners_to_dl.append("{0}/preview".format(league_metadata[key_name]))
                for banner_index in range(len(banners_to_dl)):
                    banner_url = banners_to_dl[banner_index]
                    Log("SS: Downloading {0}".format(banner_url))

                    @task
                    def DownloadImage(metadata=metadata, banner_url=banner_url, i=banner_index):
                        if banner_url not in metadata.banners:
                            ...
                            try:
                                metadata.banners[banner_url] = Proxy.Preview(
                                    get_result_from_network(banner_url, False), sort_order=(i + 1))
                            except:
            ...
            for fanart_string_index in range(1, 10):
                key_name = "strFanart{0}".format(fanart_string_index)
                if key_name in league_metadata:
                    if league_metadata.get(key_name) is not None:
                        fanart_to_dl.append(league_metadata[key_name])
                        # fanart_to_dl.append("{0}/preview".format(league_metadata[key_name]))
            for fanart_index in range(len(fanart_to_dl)):
                fanart_url = fanart_to_dl[fanart_index]
                Log("SS: Downloading {0}".format(fanart_url))

                @task
                def DownloadImage(metadata=metadata, fanart_url=fanart_url, i=fanart_index):
                    if fanart_url not in metadata.posters:
                        Log("SS: Downloading art {0}".format(fanart_url))
                        try:
                            metadata.art[fanart_url] = Proxy.Preview(
                                get_result_from_network(fanart_url, False), sort_order=(i + 1))
                        except:
